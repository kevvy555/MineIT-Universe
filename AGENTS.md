...
People and named ships are current primary image-bearing types. Purchasable ship classes may also carry image metadata and prompts. Substances may carry optional catalog-still image metadata; only records with an `image` object are in the generation process.
